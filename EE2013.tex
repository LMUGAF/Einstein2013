...
\NewEnviron{urlList}{%
	...
	%
	\colorbox{blue!15}{%
		\begin{tabularx}{\textwidth}{@{\hskip \urlListSpace}l@{\hskip \urlListSpace}X@{\hskip \urlListSpace}}%
			\noalign{\vskip 5mm}%
			\BODY%
		\end{tabularx}%
	}%
...
\NewDocumentCommand{\calHeader}{mmmmm}{%
\section*{%
...
\NewDocumentEnvironment{calDayDscNr}{mm}{%
	\begin{adjustbox}{valign=t}%
		{\fontsize{.7cm}{1em}\selectfont\sffamily\textcolor{#1}{#2}}%
	\end{adjustbox}%
}
...
\newcommand{\tikzMe}[1]{%
	\tikz[baseline]{%
		\node[anchor=base, inner sep=0pt,outer sep=0pt] (n) {\strut \noindent #1};%
		\begin{pgfonlayer}{background}%
			\fill[color=\calCellColor] (n.north west) rectangle (n.south east);%
		\end{pgfonlayer}%
	}%
}
\newcolumntype{A}[1]{%
...
\NewDocumentEnvironment{calDay}{O{black}mmO{}O{\linewidth-\tabcolsep}}{%
	\noindent
	%\makebox[#5]{\rule{#5}{1pt}}%
	\ifodd\value{page}%
		\calDayDescription{#5}{@{}Xr@{\hskip 2mm}r@{}}%
		{\calDayDscSpecial{#4}}%
		{\calDayDscName{#2}}%
		{\calDayDscNr{#1}{#3}}%
	\else%
		\calDayDescription{#5}{@{}l@{\hskip 2mm}lX@{}}%
		{\calDayDscNr{#1}{#3}}%
		{\calDayDscName{#2}}%
		{\raggedleft\calDayDscSpecial{#4}}%
	\fi%
	\minipage[t][2.2cm][b]{#5}\begingroup%
